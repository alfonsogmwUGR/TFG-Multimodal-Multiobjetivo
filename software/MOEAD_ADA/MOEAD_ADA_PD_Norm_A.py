...
class MOEAD_ADA_PD_Norm_A(MOEAD_ADA_PD):
	...
	def run(self, max_evals):
		...
		# se le asigna el j-ésimo subproblema (j-ésimo vector de pesos)
		...
		while self._evals < max_evals:
			...
			# Actualizar punto ideal z
			self.update_z(f_u)	
			norm_f_u = self.normalize_vector(f_u)
